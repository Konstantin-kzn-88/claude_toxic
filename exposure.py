...
def save_exposure(model,data,output):
    ...
    mesh=ax.pcolormesh(x,y,field,shading='auto',cmap='magma');zones=[]
    for label,threshold in criteria:
        mask=field>=threshold
        if np.nanmin(field)<threshold<np.nanmax(field):
            cs=ax.contour(x,y,field,levels=[threshold],colors='cyan');ax.clabel(cs,fmt={threshold:label})
    ...
    ax.set(title=f'Накопленная токсодоза за 0–{end:g} с; Z={z:g} м',xlabel='X по ветру, м',ylabel='Y, м')
    fig.colorbar(mesh,ax=ax,label='мг·мин/л');fig.savefig(out/'toxic_dose_xy.png',dpi=150);plt.close(fig)
    observations=[]
